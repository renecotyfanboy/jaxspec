...
class TestModules(chex.TestCase):
    ...
    def setUp(self):
        from jaxspec.model import model_components, additive_components, multiplicative_components
    ...
    def test_all_continuum(self):
        ...
        for name, module in self.module_dict:
            ...
            def f(inputs): return module().continuum(inputs)
            out = f(self.energy)
    ...
    def test_all_lines(self):
        ...
        for name, module in self.additive_dict:
            @hk.testing.transform_and_run(jax_transform=self.variant)
            def f(input_low, input_high): return module().emission_lines(input_low, input_high)

            out = f(self.energy[0:-1], self.energy[1:])
            self.assertEqual(out[0].shape, self.energy[0:-1].shape, f'{name} emission_lines change shape with flux')
            self.assertEqual(out[1].shape, self.energy[0:-1].shape, f'{name} emission_lines change shape with energies')
