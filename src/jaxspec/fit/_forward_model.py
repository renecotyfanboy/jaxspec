...
class ForwardModel(HideUnderscoreMixin, nnx.Module):
    ...
    def log_likelihood(
        ...
        *,
        instrument_params: dict | None = None,
        background_params: dict | None = None,
        observed_counts: dict[str, ArrayLike] | None = None,
    ) -> dict[str, ArrayLike]:
        """Per-bin Poisson log-likelihood for every source observation.

        Parameters:
            source_params: Optional spectral-model parameter dict.
            instrument_params: Optional instrument-calibration parameter dict.
            background_params: Optional background-model parameter dict.
            observed_counts: ``{obs_name: counts}`` to evaluate against.
                Defaults to the observed data stored in each
                :class:`~jaxspec.data.ObsConfiguration`.

        Returns:
            ``{obs_name: log_pmf_per_bin}`` dict.
        """
        from jax.scipy.stats import poisson as _poisson

        per_obs = self.expected_counts(
        ...
        out = {}
        for name, obs in self.observations.items():
            rate = per_obs[name]["total"]
            counts = (
                observed_counts[name] if observed_counts is not None else obs.folded_counts.data
            )
            out[name] = _poisson.logpmf(counts, rate)
        return out
